...
class WordsManager {
  ...
  static Set<String> wordsWithAtLeast(int nbLetters) =>
      WordsManager._instance._wordsWithAtLeast(nbLetters);
  ...
  static Set<String> findsWordsFromPermutations({
    ...
    for (final word
        in WordsManager._instance._wordsWithAtLeast(nbLettersOfSmallestWords)) {
  ...
  Set<String> _wordsWithAtLeast(int nbLetters) {
    if (!WordsManager._instance._words.keys.contains(nbLetters)) {
      // Create the cache if it does not exist
      WordsManager._instance._words[nbLetters] = WordsManager
          ._instance._words[-1]!
          .where((e) => e.length >= nbLetters)
          .toSet();
    }

    return WordsManager._instance._words[nbLetters]!;
  }
}
